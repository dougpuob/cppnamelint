...
def main():
    ...
    if   arguments['check']:
        ...
        if os.path.exists(src_dir) and os.path.exists(cfg_file):                        
            ...
            ext_name_list = [".c", ".cpp"]
            full_source_files = []
            testKit.find_files(src_dir, ext_name_list, full_source_files)
            ...
            for file_path in full_source_files:
                testKit.run_file('cppnamelint', "check " + file_path + " --config=" + os.path.abspath(cfg_file)  + " --includes=" + inc_dirs)
